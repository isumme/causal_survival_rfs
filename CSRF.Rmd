...
hist(Y[D == 1], col = adjustcolor("grey"), main = "Event vs. Censored", xlab = "Time")
...
horizons <- c(30, 60, 90, 120, 150, 180)
targets <- c("survival.probability", "RMST")

# Initialize results tibble with correct column names
results <- tibble(
  Target = character(),
  Horizon = integer(),
  Mean_C_Index = numeric()
)
...
for (target in targets) {
  for (h in horizons) {
    ...
    for (i in 1:5) {
      ...
      risk_scores <- if (target == "survival.probability") {
        1 - preds$predictions
      } else {
        -preds$predictions
      }
    ...
    results <- add_row(
      ...
      Target = target,
      Horizon = h,
      Mean_C_Index = round(mean(c_indices), 4)
    )
  }
}
