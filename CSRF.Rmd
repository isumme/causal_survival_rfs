...
# CSF Models: 30 days, 60 days, 90 days, 120 days, 150 days, 180 days

```{r}
csf_30 <- causal_survival_forest(
  X = X,
  Y = Y,
  D = D,
  W = W,
...
csf_120 <- causal_survival_forest(
  X = X,
  Y = Y,
  D = D,
  W = W,
...
csf_150 <- causal_survival_forest(
  X = X,
  Y = Y,
  D = D,
  W = W,
...
csf_180 <- causal_survival_forest(
  X = X,
  Y = Y,
  D = D,
  W = W,
...
get_summary <- function(preds, horizon) {
  s <- summary(preds)
  tibble(
    Horizon = horizon,
    Min = s["Min."],
...
cate_summary_table <- bind_rows(
  get_summary(csf_preds30$predictions, 30),
  get_summary(csf_preds60$predictions, 60),
  get_summary(csf_preds90$predictions, 90),
  get_summary(csf_preds120$predictions, 120),
  get_summary(csf_preds150$predictions, 150),
  get_summary(csf_preds180$predictions, 180)
)
